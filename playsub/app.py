...
from playsub.config import ensure_example_config, load_config, load_config_if_changed
...
class PlaysubApp:
    def __init__(self) -> None:
        ...
        self._ui_mode = ""
        self._last_ad_muted: bool | None = None
    ...
    def start(self) -> None:
        ...
        self._ui_mode = "idle"
        self.overlay.after(POLL_IDLE_MS, self.tick)
        if self.menubar is not None:
            self.overlay.after(200, self.menubar.setup_on_main_thread)
        self.overlay.run()

    def _next_poll_ms(self, playback: PlaybackState | None) -> int:
        if playback is None:
            return POLL_IDLE_MS
        if playback.is_ad:
            return POLL_AD_MS
        ...
        if (
            self.config.get("karaoke_mode", True)
            and self.current_lyrics is not None
            and self.current_lyrics.is_synced
        ):
            return POLL_KARAOKE_MS
    ...
    def tick(self) -> None:
        ...
        if now - self.last_player_poll >= PLAYER_REFRESH_SEC:
            self.config = load_config_if_changed()
        ...
        if playback is None:
            ...
            if self._ui_mode != "ad" or self._last_ad_muted != muted:
                self.overlay.show_ad(muted=muted)
                self._ui_mode = "ad"
                self._last_ad_muted = muted
                self._last_render_key = None
        else:
        ...
        self.overlay.after(self._next_poll_ms(playback), self.tick)
    ...
    def _render_track(self, position_sec: float, paused: bool) -> None:
        ...
        if self.config.get("karaoke_mode", True) and self.current_lyrics.is_synced:
            offset_ms = float(self.config.get("karaoke_offset_ms", 0))
            karaoke_words = karaoke_words_at_position(
                self.current_lyrics,
                position_sec,
                offset_sec=offset_ms / 1000.0,
            )
            active_word = next((word.text for word in karaoke_words if word.state == "active"), "")
        ...
        render_key = (
            ...
            paused,
            active_word,
            tuple((word.text, word.state) for word in (karaoke_words or ())),
        )
        if render_key == self._last_render_key:
            return
        self._last_render_key = render_key
        ...
        self.overlay.show_track(
            ...
            word_sync=self.current_lyrics.has_word_sync,
        )
